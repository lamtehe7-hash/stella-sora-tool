...
# Back/X; tap vùng trống KHÔNG đóng được (bug 2026-07-05: tap (640,150) 10 lần -> GameTooManyClick,
# popup kẹt che màn làm mọi task sau fail "không nhận diện page"). Nút "Dispatch Again" = tái phái
# (chưa code) -> đóng bằng Back, không tái phái.
...
class Dispatch(UI):
    ...
    def run(self) -> None:
        ...
        # 1. Claim nếu có đội về (Claim All sáng)
        if self.appear(COMMISSION_CLAIM_ALL_DONE):
            logger.info('Dispatch: chưa có đội về (Claim All xám)')
        else:
            self._claim_all()

        # 2. Phái lại điền slot trống (kể cả khi không claim — phòng slot còn trống từ trước)
        n = self._redispatch()
        logger.info(f'Dispatch: đã phái lại {n} commission (20h)')
        self.config.task_delay('Dispatch', minutes=240)

    def _claim_all(self) -> None:
        """Bấm Claim All rồi đóng bảng tổng kết "Commission Complete!" bằng Back (về commission page)."""
        ...
        while time.time() < end:
            ...
            if self.handle_popup():
                continue
            # Bảng tổng kết "Commission Complete!" -> đóng bằng Back (phái lại làm ở bước 2)
            if self.appear(COMMISSION_COMPLETE):
                self.device.click(COMMISSION_BACK)
                logger.info('Dispatch: đóng bảng Commission Complete (Back)')
                time.sleep(1.5)
                continue
            if self.appear(COMMISSION_CHECK):
                logger.info('Dispatch: đã Claim All commission')
                return
        ...
        logger.warning('Dispatch: Claim All không hoàn tất sau 60s — vẫn thử phái lại')
